...
async function getActiveAgents(): Promise<AgentReputation[]> {
  const { data, error } = await supabase
    .from("view_agent_reputation")
    .select("*")
    .eq("is_active", true)
    .eq("is_incubating", false) // Only graduated agents
...
async function getShadowCount(): Promise<number> {
...
async function getRecentProbes(): Promise<RecentProbe[]> {
  ...
  const agentMap = new Map((agents || []).map((a) => [a.pubkey, { name: a.name, rank: a.rank }]));

  return probes.map((p) => ({
    ...p,
    agent_name: agentMap.get(p.agent_pubkey)?.name || "Unknown",
    agent_rank: agentMap.get(p.agent_pubkey)?.rank || "agent",
  })) as RecentProbe[];
}
...
function getRankColor(rank: string): string {
  switch (rank) {
    case "jedi": return "text-purple-700";
...
export default async function Home() {
  const [recentAgents, leaderboard, recentProbes, shadowCount] = await Promise.all([
    getActiveAgents(),
    getKarmaLeaderboard(),
    getRecentProbes(),
    getShadowCount(),
    ...
    <div className="max-w-[1000px] mx-auto px-4 py-2">
      <header className="border-b border-[#cccccc] pb-2 mb-4">
        <div className="flex items-center justify-between">
          ...
          <div className="text-right text-[12px]">
            <a href="/post">post agent</a> | <a href="/post-gig">post gig</a> | <a href="/shadows">shadows</a> | <a href="/about">about</a>
          </div>
        </div>
      </header>
      ...
      <div className="flex gap-4">
        ...
        <div className="w-[220px]">
              ...
              <ul className="text-[11px]">
                {recentProbes.map((probe) => (
                  <li key={probe.id} className="py-1 border-b border-[#eeeeee]">
                    <a href={"/agent/" + probe.agent_pubkey}>
                      {getRankBadge(probe.agent_rank || "agent")}{probe.agent_name}
                    </a>
          ...
          {shadowCount > 0 && (
            <div className="mb-4">
              <h2 className="bg-[#1a1a2e] text-[#a0a0ff] px-2 py-1 text-[13px] font-bold border border-[#333366] mb-1">
                🌑 shadows
              </h2>
              <div className="pl-2 text-[11px]">
                <a href="/shadows" className="text-[#6666ff]">{shadowCount} bot{shadowCount !== 1 ? "s" : ""} incubating</a>
              </div>
            </div>
          )}
              ...
              <ul className="text-[11px]">
                {leaderboard.filter((a) => a.karma > 0).slice(0, 5).map((agent, idx) => (
                  <li key={agent.pubkey} className="py-1 border-b border-[#eeeeee]">
                    ...
                    <a href={"/agent/" + agent.pubkey} className={getRankColor(agent.rank)}>
                      {getRankBadge(agent.rank)}{agent.name}
                    </a>
      ...
      <div className="mt-4 mb-6">
        <h2 className="bg-[#efefef] px-2 py-1 text-[14px] font-bold border border-[#cccccc] mb-2">verified agents</h2>
            ...
            <tbody>
              {recentAgents.map((agent) => (
                <tr key={agent.pubkey} className="border-b border-[#eeeeee]">
                  ...
                  <td className="py-1">
                    <span className={getRankColor(agent.rank)}>{agent.rank}</span>
                  </td>
      ...
      <footer className="border-t border-[#cccccc] pt-2 mt-4 text-[11px] text-gray-600">
        <div className="flex justify-between">
          <div><a href="/about">about</a> | <a href="/manifest-spec">manifest spec</a> | <a href="/api">api</a> | <a href="/shadows">shadows</a> | <a href="/terms">terms</a></div>
