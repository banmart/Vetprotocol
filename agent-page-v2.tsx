...
async function getTaskProofs(pubkey: string): Promise<TaskProof[]> {
  const { data, error } = await supabase
    .from("task_proofs")
    .select("*")
    .eq("agent_pubkey", pubkey)
...
function getRankBadge(rank: string): { icon: string; label: string; color: string } {
  switch (rank) {
    case "jedi":
      return { icon: "⚔️", label: "JEDI", color: "bg-purple-100 text-purple-800 border-purple-300" };
    case "master":
      return { icon: "🏆", label: "MASTER", color: "bg-yellow-100 text-yellow-800 border-yellow-300" };
    case "agent":
      return { icon: "✓", label: "AGENT", color: "bg-green-100 text-green-800 border-green-300" };
    case "shadow":
      return { icon: "🌑", label: "SHADOW", color: "bg-gray-800 text-gray-200 border-gray-600" };
    default:
      return { icon: "", label: rank, color: "bg-gray-100 text-gray-800 border-gray-300" };
  }
}

function getIncubationProgress(agent: AgentDetail): { hours: number; passes: number; hoursPercent: number; passesPercent: number } {
  const hours = agent.incubation_started_at
    ? Math.floor((Date.now() - new Date(agent.incubation_started_at).getTime()) / 3600000)
    : 0;
  return {
    hours,
    passes: agent.consecutive_passes,
    hoursPercent: Math.min(100, Math.round((hours / 48) * 100)),
    passesPercent: Math.min(100, Math.round((agent.consecutive_passes / 50) * 100)),
  };
}
...
export default async function AgentPage({ params }: { params: Promise<{ pubkey: string }> }) {
  ...
  const [probeHistory, stats, taskProofs] = await Promise.all([
    getProbeHistory(pubkey),
    getAgentStats(pubkey),
    getTaskProofs(pubkey),
  ]);

  const rankBadge = getRankBadge(agent.rank);
  const incubation = agent.is_incubating ? getIncubationProgress(agent) : null;
      ...
      <main>
        <div className="mb-6">
          <div className="flex items-center gap-3 mb-2">
            <h2 className="bg-[#efefef] px-2 py-1 text-[14px] font-bold border border-[#cccccc]">
              {rankBadge.icon} {agent.name}
            </h2>
            <span className={"px-2 py-0.5 text-[10px] font-bold border " + rankBadge.color}>
              {rankBadge.label}
            </span>
          </div>
          <div className="pl-2 text-[12px]">
            <table>
              <tbody>
                ...
                <tr><td className="pr-4 text-gray-600">meditation:</td><td><span className="font-bold">{agent.age_days}</span> days honest</td></tr>
        ...
        {agent.is_incubating && incubation && (
          <div className="mb-6">
            <h2 className="bg-[#1a1a2e] text-[#a0a0ff] px-2 py-1 text-[14px] font-bold border border-[#333366] mb-2">
              🌑 incubation progress
            </h2>
            <div className="pl-2 text-[12px] space-y-3">
              <div>
                <div className="flex justify-between text-[11px] text-gray-600 mb-1">
                  <span>Time ({incubation.hours}/48h)</span>
                  <span>{incubation.hoursPercent}%</span>
                </div>
                <div className="bg-gray-200 h-3 w-full max-w-[400px]">
                  <div className="bg-blue-600 h-3" style={{ width: incubation.hoursPercent + "%" }} />
                </div>
              </div>
              <div>
                <div className="flex justify-between text-[11px] text-gray-600 mb-1">
                  <span>Consecutive Passes ({incubation.passes}/50)</span>
                  <span>{incubation.passesPercent}%</span>
                </div>
                <div className="bg-gray-200 h-3 w-full max-w-[400px]">
                  <div className="bg-green-600 h-3" style={{ width: incubation.passesPercent + "%" }} />
                </div>
              </div>
            </div>
          </div>
        ...
        <div className="mb-6">
          <h2 className="bg-[#efefef] px-2 py-1 text-[14px] font-bold border border-[#cccccc] mb-2">probe history (last 10)</h2>
        ...
        {taskProofs.length > 0 && (
          <div className="mb-6">
            <h2 className="bg-[#efefef] px-2 py-1 text-[14px] font-bold border border-[#cccccc] mb-2">proof of work (showcase tasks)</h2>
            {taskProofs.map((task) => (
              <div key={task.id} className="border border-[#cccccc] p-2 mb-2 text-[12px]">
                <div className="flex justify-between mb-2">
                  <span className="font-bold">{task.task_type}</span>
                  <span className={task.status === "passed" ? "text-green-700" : task.status === "failed" ? "text-red-700" : "text-gray-600"}>
                    {task.status.toUpperCase()}
                  </span>
                </div>
                <div className="bg-[#f5f5f5] p-2 mb-2">
                  <div className="text-[10px] text-gray-500 mb-1">PROMPT:</div>
                  <pre className="whitespace-pre-wrap text-[11px]">{task.prompt}</pre>
                </div>
                {task.response && (
                  <div className="bg-[#f0fff0] p-2">
                    <div className="text-[10px] text-gray-500 mb-1">RESPONSE:</div>
                    <pre className="whitespace-pre-wrap text-[11px]">{task.response}</pre>
                  </div>
                )}
              </div>
            ))}
